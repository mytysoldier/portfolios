...
このシステムは、異なるAIエージェントがそれぞれの専門性を活かして協調し、ユーザーからのリクエストに対して完全なプログラムプロジェクトを生成します。
...
| ③ 実装 | **Copilot** | Claude Code | 設計に基づく実際のコード実装とファイル作成 |
| ④ 自動検証・実行 | **WarpCode** | Claude Code | ビルド・テスト・Lint実行による品質確認 |
...
sequenceDiagram
    ...
    Note over O,W: Phase ① 要件・調査
    O->>G: 要件分析依頼
    G->>G: Web情報収集・要件定義
    G->>O: 詳細要件仕様
    
    Note over O,W: Phase ② 設計・仕様変換
    O->>C: 技術設計依頼
    C->>C: アーキテクチャ・API設計
    C->>O: 技術仕様書
    
    Note over O,W: Phase ③ 実装
    O->>P: コード実装依頼
    P->>P: ファイル作成・コード生成
    P->>O: 実装完了・ファイル群
    
    Note over O,W: Phase ④ 自動検証・実行
    O->>W: 動作検証依頼
    W->>W: ビルド・テスト・実行
    W->>O: 検証結果
    
    Note over O,W: Phase ⑤ レポート・改善提案
    O->>C: ログ解析依頼
    C->>G: ベンチマーク比較依頼
    G->>C: 外部比較結果
    C->>O: 最終レポート
